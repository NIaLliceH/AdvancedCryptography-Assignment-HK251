...
# Bảng tần suất chữ cái tiếng Anh (dùng làm "dấu vân tay" chuẩn)
ENGLISH_FREQUENCIES = {
...
IC_ENGLISH = 0.067 # Chỉ số trùng hợp cho tiếng Anh (gồ ghề)
IC_RANDOM = 0.038  # Chỉ số trùng hợp cho văn bản ngẫu nhiên (phẳng)

# --- BƯỚC 3: CÔNG CỤ GIẢI MÃ (Được xây dựng dựa trên sample_encrypt.py) ---

class VigenereCipher:
    """Lớp chứa logic giải mã Vigenere."""
    def __init__(self, key):
        self.key = key.lower()
        self.alphabet = ALPHABET

    def expand_key(self, text_length: int) -> str:
        """Lặp lại khóa để khớp độ dài bản mã."""
        key = self.key
        idx = 0
    ...
    def decrypt(self, ciphertext: str) -> str:
        """Giải mã ciphertext bằng khóa đã tìm được."""
        ...
        for char in ciphertext:
            if char in self.alphabet:
                ...
                # Phép TRỪ (dịch lùi) để giải mã
                plaintext += self.alphabet[(self.alphabet.index(char) - shift) % 26]
                idx += 1
            elif char in ALPHABET_UPPER:
                shift = self.alphabet.index(key[idx])
                # Phép TRỪ, sau đó chuyển thành chữ hoa
                plaintext += self.alphabet[
                    (self.alphabet.index(char.lower()) - shift) % 26
                ].upper()
                idx += 1
            else:
                # Giữ nguyên các ký tự không phải alphabet (dấu câu, khoảng trắng)
                plaintext += char
        return plaintext
...
def clean_text(text: str) -> str:
    """Chỉ giữ lại các ký tự alphabet và chuyển thành chữ thường."""
    return "".join(filter(str.isalpha, text)).lower()

def calculate_ic(text: str) -> float:
    """Tính Chỉ số Trùng hợp (Index of Coincidence) cho một đoạn văn bản."""
    N = len(text)
    if N <= 1: return 0.0
...
# --- BƯỚC 1: TÌM ĐỘ DÀI KHÓA (Phương pháp IC) ---

def find_key_length_ic(ciphertext: str, max_key_len=25) -> int:
    """
    Tìm độ dài khóa bằng phương pháp IC.
    Chọn độ dài L nào cho IC trung bình của L nhóm con gần IC_ENGLISH nhất.
    """
    cleaned_text = clean_text(ciphertext)
    key_lens_sorted = []
    
    print("--- Thử độ dài khóa (Phương pháp IC) ---")
    
    for key_length in range(2, max_key_len + 1):
        total_ic = 0.0
        
        # Chia bản mã thành key_length nhóm con ("xô")
        for i in range(key_length):
            # subgroup = cleaned_text[i::key_length] là hành động "chia xô"
            subgroup = cleaned_text[i::key_length]
            total_ic += calculate_ic(subgroup)
            
        # Tính IC trung bình (bước chuẩn hóa quan trọng)
        average_ic = total_ic / key_length
        # print(f"Độ dài {key_length:2d}: IC trung bình = {average_ic:.5f}")
        
        # abs(average_ic - IC_ENGLISH) là "độ gần" với tiếng Anh chuẩn
        key_lens_sorted.append((key_length, average_ic, abs(average_ic - IC_ENGLISH)))

    # Sắp xếp theo độ "gần" với IC tiếng Anh (giá trị thứ 3 của tuple)
    key_lens_sorted.sort(key=lambda x: x[2])
    
    print("\n--- Xếp hạng độ dài khóa tiềm năng ---")
    for length, avg_ic, diff in key_lens_sorted[:5]:
        print(f"Hạng: Độ dài = {length:2d} (IC: {avg_ic:.5f}, cách IC chuẩn: {diff:.5f})")

    best_key_len = key_lens_sorted[0][0]
    return best_key_len


# --- BƯỚC 2: TÌM KHÓA (Phân tích Tần suất Chi-squared) ---

def find_best_caesar_shift(subgroup_text: str) -> str:
    """
    Phá một mật mã Caesar (một nhóm con) bằng phép thử Chi-squared.
    Tìm shift nào cho "độ vênh" (chi_squared_sum) thấp nhất.
    """
    best_shift = 0
    ...
    # Thử 26 khả năng dịch lùi (shift = 0 đến 25)
    for shift in range(26):
        chi_squared_sum = 0.0
        
        # Dịch ngược (decrypt) nhóm con với "phép đoán" shift hiện tại
        ...
        # Tính toán Chi-squared (đo độ vênh)
        for i in range(26):
            char = ALPHABET[i]
            # Tính "Kỳ vọng" (Expected)
            expected_count = ENGLISH_FREQUENCIES[char] * text_len
            # Tính "Thực tế" (Observed)
            ...
            # Công thức Chi-squared: (Thực tế - Kỳ vọng)^2 / Kỳ vọng
            chi_squared_sum += (difference * difference) / expected_count
            
        # Tìm "độ vênh" nhỏ nhất -> Đây là shift chính xác
        if chi_squared_sum < min_chi_squared:
            min_chi_squared = chi_squared_sum
            best_shift = shift
            
    # Trả về chữ cái khóa tương ứng với độ dịch tốt nhất
    return ALPHABET[best_shift]

# --- CHƯƠNG TRÌNH CHÍNH (Điều phối cuộc tấn công) ---

if __name__ == "__main__":
    
    # Bản mã thử thách được cung cấp trong đề bài
    ...
    # --- BƯỚC 1: TÌM ĐỘ DÀI KHÓA ---
    # Độ dài khóa tối đa được đặt là 25
    key_length_guess = find_key_length_ic(challenge_ciphertext, max_key_len=25)
    
    if key_length_guess == 0:
        print("Không tìm thấy độ dài khóa phù hợp. Thử tăng max_key_len.")
    else:
        print(f"\n==> Độ dài khóa tiềm năng nhất là: {key_length_guess}\n")

        # --- BƯỚC 2: TÌM KHÓA ---
        print("--- Bước 2: Tìm Khóa (Phân tích Tần suất) ---")
        
        found_key = ""
        cleaned_ciphertext_for_groups = clean_text(challenge_ciphertext)
        
        # Lặp L lần, mỗi lần tìm 1 chữ cái khóa
        for i in range(key_length_guess):
            ...
            found_key += key_char
            print(f"Nhóm {i+1:2d}/{key_length_guess}: Chữ cái khóa tìm được = '{key_char}'")
            
        print(f"\n==> Khóa bí mật tìm được (dự đoán): '{found_key}'\n")

        # --- BƯỚC 3: GIẢI MÃ ---
        print("--- Bước 3: Giải mã ---")
        cipher_solver = VigenereCipher(found_key)
        recovered_plaintext = cipher_solver.decrypt(challenge_ciphertext)
        
        print(f"Plaintext (đã giải mã):\n")
